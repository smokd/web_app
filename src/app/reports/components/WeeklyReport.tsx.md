...
export default function WeeklyReport({ data }: { data: Harvest[] }) {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      fieldRejectPct: harvested > 0 ? (fieldReject / harvested) * 100 : 0,

      packhouseRejectPct:
        processed > 0 ? (packhouseReject / processed) * 100 : 0,

      totalLoss: fieldReject + packhouseReject,

      overallRejectPct:
        harvested > 0 ? ((fieldReject + packhouseReject) / harvested) * 100 : 0,
    };
  }, [data]);
  ...
  0,
    ...
    return Array.from(map.entries())
      .map(([variety, values]) => ({
        ...
        fieldRejectPct:
          values.harvested > 0
            ? (values.fieldReject / values.harvested) * 100
            : 0,

        processed: values.processed,

        packhouseReject: values.packhouseReject,

        packhouseRejectPct:
          values.processed > 0
            ? (values.packhouseReject / values.processed) * 100
            : 0,
      }))
  ...
  const defectData = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([name, values]) => ({
        ...
        packhouseKg: values.packhouseKg,

        totalKg: values.fieldKg + values.packhouseKg,
      }))
    ...
    <div className="weekly-report">
      ...
      <div className="report-tabs">
        <button
          ...
          className={view === "overview" ? "report-tab active" : "report-tab"}
        >
          Overview
        </button>
        ...
        <button
          ...
          className={view === "variety" ? "report-tab active" : "report-tab"}
        >
          By Variety
        </button>
        ...
        <button
          ...
          className={view === "daily" ? "report-tab active" : "report-tab"}
        >
          Daily Trend
        </button>
        ...
        <button
          ...
          className={view === "defects" ? "report-tab active" : "report-tab"}
        >
          Defects
        </button>
      </div>
      ...
      <ReportCard title="Daily Harvest & Processing">
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={dailyData}>
            ...
            <Bar dataKey="harvested" name="Harvested Kg" fill="#0ac5b2" />

            <Bar dataKey="processed" name="Processed Kg" fill="#2c7ec4" />
          </BarChart>
        </ResponsiveContainer>
      </ReportCard>
...
function VarietyReport({ data }: { data: any[] }) {
  return (
    <div className="report-section">
      <ReportCard title="Harvest Performance by Variety">
        <ResponsiveContainer width="100%" height={400}>
          <BarChart data={data}>
            ...
            <Bar dataKey="fieldReject" name="Field Reject Kg" fill="#ef4444" />
...
function DefectReport({ data }: { data: any[] }) {
  return (
    <div className="report-section">
...
function DefectTable({ data }: { data: any[] }) {
  return (
    <div className="report-table-wrapper">
      <table className="report-table">
        <thead>
