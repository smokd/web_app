...
export default async function PackhousePage() {
  ...
  function formatDate(value: unknown) {
    ...
    if (!match) {
      console.error("UNRECOGNIZED DATE FORMAT:", raw);
  ...
  const totalAwaitingKg = awaitingPackhouse.reduce(
    (sum, harvest) => sum + Number(harvest.harvestedKg || 0),
    0,
  );
    ...
    <main className="packhouse-page">
      ...
      <header className="packhouse-header">
        <div>
          ...
          <p>Receive, process and record packhouse quality information.</p>
        </div>
      ...
      <section className="dashboard-kpis">
        <div className="dashboard-kpi">
          ...
          <strong>{awaitingPackhouse.length}</strong>

          <small>Harvest records</small>
        </div>

        <div className="dashboard-kpi">
          <span>Awaiting Quantity</span>

          <strong>
            {totalAwaitingKg.toLocaleString(undefined, {
        ...
        <div className="dashboard-kpi">
          ...
          <strong>{processedHarvests.length}</strong>
        ...
        <div className="dashboard-kpi">
          ...
          <strong>
            {totalProcessedKg.toLocaleString(undefined, {
              maximumFractionDigits: 2,
            })}{" "}
            kg
          </strong>

          <small>Packhouse throughput</small>
        </div>
      </section>
      ...
      <section className="packhouse-section">
        <div className="section-heading">
          <div>
            <h2>Awaiting Packhouse Processing</h2>

            <p>Harvest records requiring packhouse processing.</p>
          </div>

          <div className="record-count">
            {awaitingPackhouse.length}{" "}
            {awaitingPackhouse.length === 1 ? "record" : "records"}
          </div>
        </div>
        ...
        {awaitingPackhouse.length === 0 ? (
          <div className="packhouse-empty">
            ...
            <p>
              There are no harvest records waiting for packhouse processing.
            </p>
          </div>
        ) : (
          <div className="packhouse-queue">
            {awaitingPackhouse.map((harvest) => {
              const fieldRejectKg = Number(harvest.fieldRejectsKg || 0);

              const fieldRejectPct =
                harvest.harvestedKg > 0
                  ? (fieldRejectKg / harvest.harvestedKg) * 100
                  : 0;

              return (
                <article key={harvest.id} className="packhouse-queue-card">
                  ...
                  <div className="packhouse-record-summary">
                    <div>
                      ...
                      <strong>
                        {Number(harvest.harvestedKg).toFixed(2)} kg
                      </strong>
                    </div>

                    <div>
                      <span>Field Reject</span>

                      <strong>{fieldRejectKg.toFixed(2)} kg</strong>

                      <small>{fieldRejectPct.toFixed(2)}%</small>
                    </div>

                    <div>
                      <span>Supervisor</span>

                      <strong>{harvest.supervisor || "—"}</strong>
                    </div>
                  </div>
                  ...
                  <div className="packhouse-process-area">
                    <PackhouseForm
                      ...
                      harvestedKg={Number(harvest.harvestedKg)}
      ...
      <section className="packhouse-section">
        <div className="section-heading">
          <div>
            ...
            <p>Packhouse processing records already completed.</p>
          </div>

          <div className="record-count">
            {processedHarvests.length}{" "}
            {processedHarvests.length === 1 ? "record" : "records"}
          </div>
        </div>

        {processedHarvests.length === 0 ? (
          <div className="packhouse-empty">
            <strong>No processed records</strong>

            <p>Packhouse processing records will appear here.</p>
          </div>
        ) : (
