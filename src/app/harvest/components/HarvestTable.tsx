...
export default function HarvestTable({
  ...
  return (
    <div>
      ...
      {message && (
        <p style={{ marginBottom: '1rem', color: message.includes('deleted') || message.includes('updated') ? '#2e7d32' : '#c62828' }}>
          {message}
        </p>
      )}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid var(--border)' }}>
              ...
              <th style={{ textAlign: 'right', padding: '0.6rem' }}>Harvested</th>
              <th style={{ textAlign: 'right', padding: '0.6rem' }}>Field Rej</th>
              <th style={{ textAlign: 'right', padding: '0.6rem' }}>Field %</th>
              <th style={{ textAlign: 'right', padding: '0.6rem' }}>Packhouse</th>
              <th style={{ textAlign: 'right', padding: '0.6rem' }}>Packhouse %</th>
              <th style={{ textAlign: 'left', padding: '0.6rem' }}>Blocks</th>
              <th style={{ textAlign: 'left', padding: '0.6rem' }}>Weather</th>
              <th style={{ textAlign: 'left', padding: '0.6rem' }}>Supervisor</th>
  ...
  : '—'}
                    ...
                    <td style={{ padding: '0.6rem' }}>
                      {record.weather ? `${record.weather}${record.weatherTemp ? ` (${fmt2(record.weatherTemp)}°C)` : ''}` : '—'}
                    </td>
                    <td style={{ padding: '0.6rem' }}>{record.supervisor || '—'}</td>
                      ...
                      <td style={{ padding: '0.6rem' }}>
                        <div style={{ display: 'flex', gap: '0.5rem' }} onClick={(e) => e.stopPropagation()}>
                          ...
                          <button
                            ...
                            disabled={pending}
                            style={{ padding: '0.3rem 0.6rem', fontSize: '0.85rem', background: '#c62828', color: '#fff' }}
                          >
                            Delete
                          </button>
                        </div>
                  ...
                  {expandedId === record.id && (
                    <tr>
                      <td colSpan={isAdmin ? 12 : 11} style={{ padding: '1rem', background: 'rgba(0,0,0,0.02)' }}>
                        {record.fieldRejects.length > 0 && (
                          <div style={{ marginBottom: '0.75rem' }}>
                            <strong style={{ fontSize: '0.85rem' }}>Field Defects:</strong>
...
function EditRow({
  ...
  const rejectPct = harvestedNum > 0 ? ((rejectsNum / harvestedNum) * 100).toFixed(2) : '0.00';

  return (
    <tr style={{ borderBottom: '1px solid var(--border)', background: 'rgba(255,255,255,0.05)' }}>
      <td colSpan={isAdmin ? 12 : 11} style={{ padding: '0.6rem' }}>
        <form action={onSubmit} style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end' }}>
          <input type="hidden" name="id" value={record.id} />
          <input type="hidden" name="date" value={record.date} />

          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 120 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Variety</label>
            <select name="variety" defaultValue={record.variety} required style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }}>
              {varieties.map((v) => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 100 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Harvested</label>
            <input name="harvestedKg" type="number" min="0" step="0.01" required value={harvested} onChange={(e) => setHarvested(e.target.value)} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 100 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Rejects</label>
            <input name="fieldRejectsKg" type="number" min="0" step="0.01" required value={rejects} onChange={(e) => setRejects(e.target.value)} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 80 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Rej %</label>
            <div style={{ padding: '0.4rem', borderRadius: 4, background: 'var(--formula-bg, #e8f5e9)', fontWeight: 600, minWidth: 60 }}>
              {rejectPct}%
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 100 }}>
            ...
            <input name="blocks" type="text" defaultValue={record.blocks || ''} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 120 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Weather</label>
            <select name="weather" defaultValue={record.weather || ''} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }}>
              <option value="">—</option>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 120 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Supervisor</label>
            <input name="supervisor" type="text" defaultValue={record.supervisor || ''} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', minWidth: 200, flex: 1 }}>
            <label style={{ fontSize: '0.75rem', opacity: 0.7 }}>Notes</label>
            <input name="notes" type="text" maxLength={500} defaultValue={record.notes || ''} style={{ padding: '0.4rem', borderRadius: 4, border: '1px solid var(--border)' }} />
          </div>

          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button type="submit" disabled={pending} className="btn btn-primary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem', opacity: pending ? 0.6 : 1 }}>
              {pending ? 'Saving…' : 'Save'}
            </button>
